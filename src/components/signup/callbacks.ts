...
export const inputDataValidate: Callback = () => {
  ...
  const passwordInput = <HTMLInputElement>(
    document.getElementsByClassName("user-box__password")[0]
  );
  const repasswordInput = <HTMLInputElement>(
    document.getElementsByClassName("user-box__confirm-password")[0]
  );
...
export const showAlert: Callback = (obj: { error: string }) => {
  ...
  const alertLabel = <HTMLLabelElement>(
    document.getElementsByClassName("auth-content-inner__error")[0]
  );
...
export const handleSignUpDenied: Callback = (obj: { responseText: string }) => {
  ...
  Promise.resolve()
    ...
    .then((value) =>
      bus.emit("show alert", { error: value["error description"] })
    )
